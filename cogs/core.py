...
#                      BROADCAST COMMAND
# =====================================================================
    @commands.command(name="broadcast",aliases=["bcm"])
    @commands.check(lambda ctx: ctx.author.id in OWNER_IDS)
    async def broadcast(self, ctx: commands.Context, *, message: str | None = None):
        """
        Broadcast a message embed to all guilds where the bot can post.
        Usage: ;broadcast <text>
        """
        if not message:
            return await ctx.reply(embed=mkembed("❌ Missing message", "Usage: `;broadcast <text>`", COLORS["ERROR"]))

        confirm = mkembed(
            "📣 Broadcast Confirmation",
            f"This will send your message to **{len(self.bot.guilds)} servers**.\n\n"
            f"**Message:**\n> {message}\n\nType `yes` to confirm within 20s.",
            COLORS["WARNING"]
        )
        ...
        try:
            ...
            if msg.content.lower() != "yes":
                return await ctx.reply(embed=mkembed("❌ Cancelled", "Broadcast cancelled.", COLORS["ERROR"]))
        except asyncio.TimeoutError:
            return await ctx.reply(embed=mkembed("⌛ Timeout", "Broadcast cancelled (no confirmation).", COLORS["ERROR"]))

        # Build the embed to send
        broadcast_embed = mkembed("# 📢 Announcement", f"## {message}", COLORS["INFO"])
        broadcast_embed.add_field(name="Disclaimer",value="-# This is a broadcast message.", inline=False)
        broadcast_embed.set_footer(text=f"- Regards, \n The Helix Team")

        sent = 0
        failed = 0

        await ctx.reply(embed=mkembed("📡 Broadcasting...", "Sending message to all servers...", COLORS["INFO"]))
        ...
        for guild in self.bot.guilds:
            ...
            if guild.system_channel and guild.system_channel.permissions_for(guild.me).send_messages:
                ...
                # Fallback to first accessible text channel
            ...
            try:
                ...
                sent += 1
                await asyncio.sleep(1.0)  # small delay to avoid rate limits
            except Exception:
                failed += 1
                continue
        ...
        summary = mkembed(
            "✅ Broadcast Complete",
            f"Message sent to **{sent} servers**.\nFailed to send to **{failed} servers**.",
            COLORS["SUCCESS"] if failed == 0 else COLORS["WARNING"]
        )
